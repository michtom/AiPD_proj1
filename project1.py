...
class GUI:
    ...
    def __init__(self):
        ...
        ttk.Label(self.window, text="Select value to be displayed in red").grid(row=6, column=4)
        ttk.Radiobutton(self.window, text="silence",
                        variable=self.value_red, value=1, command=self.choose_plot).grid(row=7, column=4, sticky="we")
        ttk.Radiobutton(self.window, text="voiced frames",
                        variable=self.value_red, value=2, command=self.choose_plot).grid(row=8, column=4, sticky="we")
